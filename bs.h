...
static inline uint32_t bs_read_u8_helper(bs_t* b)
{
    if (! bs_eof(b)) // can do fast read
    {
        uint32_t r = b->p[0];
        b->p++;
        return r;
...
// n <= b->bits_left
static inline uint32_t bs_read_u_bits_helper(bs_t* b, int n)
{
    uint32_t r = 0;
    r = (b->p[0] >> (8 - b->bits_left)) & ((1 << n) - 1);
    b->bits_left -= n;
    if (b->bits_left == 0)
    {
        b->bits_left = 8;
...
static inline uint32_t bs_read_u(bs_t* b, int n)
{
    ...
    if (n > b->bits_left)
    {
        int pos = b->bits_left;
        n -= b->bits_left;
        r = bs_read_u_bits_helper(b, b->bits_left);
        while (n >= 8)
        {
            // read whole byte and align to "pos"
            r = r | (bs_read_u8_helper(b) << pos);
            n -= 8;
            pos += 8;
        }
        if (n > 0)
        {
            // n must be smaller than 8
            r = r | (bs_read_u_bits_helper(b, n) << pos);
        }
    }
    else
    {
        r = bs_read_u_bits_helper(b, n);
    }
    return r;
}
#else
