 ...
 ***********************************************************************
 */ 
int intrapred_luma_16x16(ColorPlane pl, struct img_par *img, //!< image parameters
...
void itrans_2(ColorPlane pl, struct img_par *img) //!< image parameters
{
  ...
  for (i=0;i<4;i++)
  {
    M5[0]=img->cof[i][0+uv_idx][0][0]; 
    M5[1]=img->cof[i][1+uv_idx][0][0];  
    M5[2]=img->cof[i][2+uv_idx][0][0]; 
    M5[3]=img->cof[i][3+uv_idx][0][0]; 
    ...
    if (pl)
    {  
      img->cof[i][0+uv_idx][0][0] = rshift_rnd((((M6[0]+M6[3])*InvLevelScale4x4Chroma_Intra[uv][qp_rem][0][0]) << qp_per), 6); 
      img->cof[i][1+uv_idx][0][0] = rshift_rnd((((M6[1]+M6[2])*InvLevelScale4x4Chroma_Intra[uv][qp_rem][0][0]) << qp_per), 6); 
      img->cof[i][2+uv_idx][0][0] = rshift_rnd((((M6[1]-M6[2])*InvLevelScale4x4Chroma_Intra[uv][qp_rem][0][0]) << qp_per), 6); 
      img->cof[i][3+uv_idx][0][0] = rshift_rnd((((M6[0]-M6[3])*InvLevelScale4x4Chroma_Intra[uv][qp_rem][0][0]) << qp_per), 6);
    }
    else
    {
...
void iMBtrans4x4(ColorPlane pl, struct img_par *img, int smb)
{
  ...
  { 
    for (k = block8x8; k < block8x8 + 4; k ++)
    {
      ...
      {
        if (pl)
          itrans(img, (i << BLOCK_SHIFT), (j << BLOCK_SHIFT), i, j+uv_idx, 1, uv+1);
        else
      ...
      for(j = 0; j < BLOCK_SIZE; j++)
      {
        ...
        {
          if (pl)
            dec_picture->imgUV[uv][j_pos + j][i_pos + i] = img->m7[j][i]; // construct picture from 4x4 blocks
          else 
...
void iMBtrans8x8(ColorPlane pl, struct img_par *img)
{
  ...
  for (block8x8=0; block8x8<4; block8x8++)
  {
    ...
    for(j = joff; j < joff + 8; j++)
    {
      ...
      {
        if (pl)
          dec_picture->imgUV[uv][img->pix_y + j][img->pix_x + i] = img->m7_CbCr[uv][j][i];
        else 
...
void iTransform(ColorPlane pl, struct img_par *img, Macroblock *currMB, int need_4x4_transform, int smb, int yuv)
{
  ...
  else
  {
    ...
    { 
      if (pl)
        memcpy(&(dec_picture->imgUV[uv][img->pix_y + j][img->pix_x]), &(mpr[j][0]), MB_BLOCK_SIZE * sizeof(imgpel));
      else
