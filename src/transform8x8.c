 ...
 ************************************************************************
 */
int intrapred8x8( ColorPlane pl, struct img_par *img,  //!< image parameters
                  int b8)
              ...
              int joff)            //!<
{
  ...
  if(lossless_qpprime)
  {
    for( j = joff; j < joff + 8; j++)
    {
      for( i = ioff; i < ioff + 8; i++)
        if (pl)
          img->m7_CbCr[uv][j][i] = iClip1(img->max_imgpel_value, (img->m7_CbCr[uv][j][i]+(long)mpr[j][i])); 
        else
  ...
  else
  {
    ...
    for( i = 0; i < 8; i++)
    {
      ...
      if (pl) 
      {
        img->m7_CbCr[uv][joff + 0][ipos] = b[0] + b[7]; 
        img->m7_CbCr[uv][joff + 1][ipos] = b[2] + b[5];
        img->m7_CbCr[uv][joff + 2][ipos] = b[4] + b[3];
        img->m7_CbCr[uv][joff + 3][ipos] = b[6] + b[1];
        img->m7_CbCr[uv][joff + 4][ipos] = b[6] - b[1];
        img->m7_CbCr[uv][joff + 5][ipos] = b[4] - b[3];
        img->m7_CbCr[uv][joff + 6][ipos] = b[2] - b[5];
        img->m7_CbCr[uv][joff + 7][ipos] = b[0] - b[7];
      }
      else
      {
    ...
    for( j = joff; j < joff + 8; j++)
    {
      for( i = ioff; i < ioff + 8; i++)
        if (pl)  
        {
          img->m7_CbCr[uv][j][i] = iClip1(img->max_imgpel_value,  
            rshift_rnd_sf((img->m7_CbCr[uv][j][i]+((long)mpr[j][i] << DQ_BITS_8)),DQ_BITS_8)); 
        }
        else
        {
