...
Problem Statement
    
Warning: This problem statement contains superscripts and/or subscripts. It may not display properly outside of the applet.  You are playing a video game similar to the famous traveling salesman problem. In this game, you travel between cities and collect profits according to some rules explained in the next paragraphs. Unlike the traditional traveling salesman problem, it is allowed to visit each of the cities multiple times.  There are N cities numbered 0 through N-1. Each city has an associated value. The values are given in vector <int> cityValues, where cityValues[i] is the value of city i.  There are also zero or more bidirectional roads connecting the cities. Each road connects exactly two different cities and can be traversed in both ways. The information about the roads is given in vector <string> roads. The j-th character of the i-th element of roads will be 'N' if there is no road connecting city i and city j, or 'Y' if there is exactly one road connecting city i and city j.  In this game, you start at city 0 with profit cityValues[0]. Your goal is to maximize your final profit. At any time in the game, you may perform one of the following actions:  
...
/*
void copySet(set<int>& dst, const set<int>& src)
{
	dst.clear();
	cout << "size is "<< src.size() << endl;
...
typedef set<Node, NodeComp> NodeSet;
...
private:
	...
	void addCity(const vector<string>& roads, const Node& node)
	{
		int city = node.city;
		const string& road = roads[city];
		for (int i = 0; i < road.size(); i ++)
		{
			if (node.city != i && road[i] == 'Y')
			{
				Node newNode;
				newNode.city = i;
				copySet(newNode.states, node.states);
				if (newNode.states.find(i) == newNode.states.end())
				{
					newNode.states.insert(i);
				}
				else
				{
					newNode.states.erase(i);
				}
	...
	int calculateProfit(const vector<int>& cityValues, const set<int>& cities)	
	{
		int result = 0;
		for (set<int>::const_iterator it = cities.begin(); it != cities.end(); it ++)
		{
			result = result ^ cityValues[*it];
		}
		return result;
	}
public:
	int maxProfit(vector<int> cityValues, vector<string> roads)
	{
		...
		{
			int profit = calculateProfit(cityValues, it->states);
			if (profit > result)
			{
				result = profit;
			}
...
{
private:
	set<pair<int, int> > cityMap;
	int cityCount;
	vector<int> m_cityValues;
	vector<string> m_roads;
	int findMax(int city, int profit)
	{
		int result = profit;
		cityMap.insert(make_pair(city, profit));
		...
		for (int i = 0; i < cityCount; i ++)
		{
			if (i != city)
			{
				if (m_roads[city][i] == 'Y')
				{
					int tmp = profit ^ m_cityValues[i];
					if (cityMap.find(make_pair(i, tmp)) == cityMap.end())
					{
						int ret = findMax(i, tmp);
						if (ret > result)
						{
							result = ret;
						}
...
public:
	int maxProfit(vector<int> cityValues, vector<string> roads)
	{
		cityCount = cityValues.size();
		m_cityValues.assign(cityValues.begin(), cityValues.end());
		m_roads.assign(roads.begin(), roads.end());

		return findMax(0, cityValues[0]);
	}
};
const char* roadArray[] =
{
		 "NNNNNNYYYYNNNNNNNNNNNNNNNNNNNNNNNNNYNNNNNNNNNNNNNN",
		 "NNNNNNNNNNNNNNNNYNNNNNNNYNNNNNNNNNNNNNNNNYYNNNYYNN",
		 "NNNNNYYNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN",
...
{
	const size_t Size = 1;
	vector<int> cityValue;
	vector<string> roads;
	for (size_t i = 0; i < Size; i ++)
	{
		cityValue.push_back(rand()%100 +1);
		string road;
		for (size_t j = 0; j < Size; j ++)
		{
			road.push_back(rand()%2==0 ? 'Y' : 'N');
		}
		roads.push_back(road);
	}
	XorTravelingSalesman man;
	cout << man.maxProfit(cityValue, roads) << endl;
}
static int test2()
{
	int cityValueArray[] = {37, 1, 19, 64, 42, 41, 64, 64, 54, 16, 256, 36, 64, 2, 4, 2, 62, 29, 58, 64, 1, 32, 16,
			 256, 17, 2, 17, 4, 1, 64, 21, 8, 256, 63, 3, 1, 43, 15, 8, 39, 41, 8, 16, 8, 16, 256, 64, 512, 45, 64};
	vector<int> cityValues(cityValueArray, cityValueArray + sizeof(cityValueArray)/sizeof(int));

	vector<string> roads(roadArray, roadArray+cityValues.size());
	XorTravelingSalesman man;
	cout << man.maxProfit(cityValues, roads) << endl;
	return 0;
}
