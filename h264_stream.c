...
static inline int find_nal_prefix(uint8_t* buf, int size, int start)
{
    int i = start;
    while (i < size)
    {
        ...
        {
            if (buf[i+2] == 0 && buf[i+3] == 0x01 && i+3 < size)
            {
                return i;
            }
            if (buf[i+2] == 0x01 && i+2 < size)
            {
                return i;
            }
            else
...
int
more_rbsp_trailing_data(h264_stream_t* h, bs_t* b)
{
    return !bs_eof(b);
}
